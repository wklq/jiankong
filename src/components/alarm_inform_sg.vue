        ...
        <el-scrollbar style="height:350px;" class="loncom_scrollbar">
            <div class="loncom_dialog_con">
                <el-form :model="form_info" :rules="formRules" ref="formInfo" label-width="100px">
                    <el-form-item prop="port" label="通信串口">
...
export default {
    ...
    mounted() {
        this.$api.get('/Alarmstrategy/getAlarmDevsetting', {"alarmdevid":3}, r => {
            console.log(r)
            if(r.err_code=="0"){
                var comsetting=r.data[0].comsetting;
                this.form_info.port=comsetting.split(":")[0];
                this.form_info.type=comsetting.split(":")[1];
            }
        });
    },
    data() {
        ...
        dialogSure:function(formName){
            this.$refs[formName].validate((valid) => {
                if(valid){
                    ...
                    this.$api.post('/Alarmstrategy/UpdateAlarmDevsetting', {"alarmdevid":3,"setting":this.form_info.port+":"+this.comType}, r => {
                        console.log(r)
                        if(r.err_code=="0"){
